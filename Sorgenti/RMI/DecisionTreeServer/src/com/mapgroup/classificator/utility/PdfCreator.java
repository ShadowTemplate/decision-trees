 ...
 * (0, 0) is the bottom-left corner of the pdf page <br>(0, 780) is the
 * top-left corner of the pdf page <br>(600, 0) is the bottom-right corner of
 * the pdf page <br>(600, 780) is the top-right corner of the pdf page
 * </p>
 */
public class PdfCreator {
     ...
     */
    private static void writeText(Color c, PDFont f, String msg)
            throws IOException {
        contentStream.beginText();
        contentStream.setFont(f, currFontDim);
    ...
    public static void createReport(PredSessionTO tracker, String pdfFileName)
        ...
        */
        ...
        newLines(2);
        currFontDim = textDim;
        HashMap<String, String> map = tracker.getTracker();
        ...
        for (String key : keys) {
            ...
            if (key.length() < 53) {
                ...
                contentStream.endText();
                newLines(1);
                writeText(Color.BLACK, textFont, lines[0]);

                for (int j = 1; j < lines.length - 1; j++) {
                    newLines(1);
                    writeText(Color.BLACK, textFont, lines[j]);
                }

                if (lines.length > 1) {
                    newLines(1);
                    writeText(Color.BLACK, textFont, lines[lines.length - 1]);
                }

                newLines(1);
                writeText(Color.BLACK, textFont,
                        "// Choice = " + map.get(key));
            }
        ...
        newLines(2);
        currFontDim = textDim;
        writeText(Color.BLUE, textFont, tracker.getPredictedValue());
        ...
        for (String s : treeStr) {
            if (s.length() < 115) {
                writeText(Color.BLACK, textFont, s);
                newLines(1);
            } else {
